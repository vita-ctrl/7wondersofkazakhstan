...
export default function ReviewCard({ logo, name, rating, text }) {
  return (
    <div className="bg-cream dark:bg-gray-800 backdrop-blur-lg shadow-xl rounded-3xl p-5 border border-white/50 hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 group relative overflow-hidden">
      ...
      <div className="flex items-center gap-3 mb-4 relative z-10">
        <div className="relative">
          <img
            src={logo}
            alt={name}
            className="w-12 h-12 rounded-full shadow-lg object-cover border-2 border-beige group-hover:scale-110 transition-transform duration-300"
          />
          <div className="absolute -bottom-1 -right-1 w-5 h-5 bg-linear-to-r from-yellow-400 to-orange-500 rounded-full border-2 border-beige flex items-center justify-center">
      ...
      <p className="text-gray-600 dark:text-beige text-sm leading-relaxed line-clamp-4 group-hover:line-clamp-none transition-all duration-300">
        {text}
      </p>
    </div>
  );
